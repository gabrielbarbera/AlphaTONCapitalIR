/* AlphaTON Capital IR Website - Stock Chart Component */
...
class StockChart {
    ...
    try {
      ...
      for (const api of apis) {
        try {
          ...
          if (api.name === "YAHOO_FINANCE") {
            await this.fetchYahooFinanceData();
          } else if (api.name === "YAHOO_FINANCE_ALT") {
            await this.fetchYahooFinanceAltData();
  ...
  getEnabledApis() {
    ...
    if (API_CONFIG.YAHOO_FINANCE.ENABLED) {
      apis.push({
        name: "YAHOO_FINANCE",
        priority: API_CONFIG.YAHOO_FINANCE.PRIORITY,
      });
    }

    if (API_CONFIG.YAHOO_FINANCE_ALT.ENABLED) {
      apis.push({
        name: "YAHOO_FINANCE_ALT",
        priority: API_CONFIG.YAHOO_FINANCE_ALT.PRIORITY,
      });
    }
  ...
  async fetchYahooFinanceData() {
    const interval = this.getYahooInterval();
    const range = this.getYahooRange();

    const url = `${API_CONFIG.YAHOO_FINANCE.BASE_URL}/${this.symbol}?interval=${interval}&range=${range}`;

    console.log("Fetching Yahoo Finance data from:", url);
    ...
    this.chartData = this.processYahooData(data.chart.result[0]);
  ...
  async fetchYahooFinanceAltData() {
    const interval = this.getYahooInterval();
    const range = this.getYahooRange();

    const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${this.symbol}?interval=${interval}&range=${range}`;
    const url = `${
      API_CONFIG.YAHOO_FINANCE_ALT.BASE_URL
    }&url=${encodeURIComponent(yahooUrl)}`;

    console.log("Fetching Yahoo Finance Alt data from:", url);

    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`Yahoo Finance Alt API error: ${response.status}`);
    }

    const data = await response.json();
    console.log("Yahoo Finance Alt Response:", data);

    if (!data.chart || !data.chart.result || data.chart.result.length === 0) {
      throw new Error("No data available from Yahoo Finance Alt");
    }
...
function updateChart(timeframe) {
  if (stockChartInstance) {
    stockChartInstance.updateTimeframe(timeframe);
  }
}

function loadStockData() {
  if (stockChartInstance) {
    stockChartInstance.loadStockData();
  }
}

// Initialize when DOM is ready
document.addEventListener("DOMContentLoaded", function () {
  stockChartInstance = new StockChart();
});
